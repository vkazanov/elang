...
(defun parse-small-stmt ()
  (cond
   ((token-is-p 'NAME flow-val-firstset)
    (parse-flow-stmt))
   ((token-is-p 'NAME '("assert"))
    (parse-assert-stmt))
   ((token-is-p 'NAME '("pass"))
    (token-pop)
    nil)
   ((or (token-one-of-p expr-type-firstset)
        (token-is-p 'NAME expr-val-firstset))
    (parse-expr-stmt))
   (t (throw 'parser-error "Unexpected token"))))
...
(defun parse-simple-stmt ()
  (let ((small-stmts (list (parse-small-stmt))))
    ...
    (while (or (token-one-of-p small-type-firstset)
               (token-is-p 'NAME small-val-firstset))
   ...
   ((token-is-p 'NEWLINE)
    ...
    (let ((stmtlist (list (parse-simple-stmt))))
      (while (or (token-one-of-p stmt-type-firstset)
                 (token-is-p 'NAME stmt-val-firstset))
...
(defun parse-stmt ()
  (cond
   ((token-is-p 'NAME compound-stmt-val-firstset)
    (parse-compound-stmt))
   ((or (token-is-p 'NAME simple-val-firstset )
